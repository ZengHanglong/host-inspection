    ...
    <div class="page-header">
      <div class="header-content">
        <div class="title-group">
          ...
          <h1 class="page-title">大容量虚拟机排查</h1>
        </div>
        <div class="schedule-badge">
          ...
          <span>每季度 - 最后一个工作日</span>
        </div>
      </div>
      <div class="header-rules">
        <div class="rule-item">
          <span class="rule-label">阈值</span>
          <span class="rule-value">使用空间超过1TB的虚拟机</span>
        </div>
        <div class="rule-item">
          <span class="rule-label">规则</span>
          <span class="rule-value">定位大容量虚拟机，看是否涉及清理</span>
        </div>
      </div>
    </div>
    ...
    <div class="stats-section">
      <div class="stat-card glass-card large">
        ...
        <div class="stat-content">
          ...
          <span class="stat-label">大容量虚拟机数</span>
        </div>
        <div class="stat-pulse" v-if="data.total > 0"></div>
      </div>

      <div class="stat-card glass-card threshold">
        <div class="stat-icon">
          <svg viewBox="0 0 24 24" fill="none">
        ...
        <div class="stat-content">
          <span class="stat-value">{{ data.threshold_tb }}</span>
          <span class="stat-unit">TB</span>
          <span class="stat-label">容量阈值</span>
        </div>
      </div>
      ...
      <div class="stat-card glass-card healthy" v-if="data.total === 0">
        ...
        <div class="stat-content">
          ...
          <span class="stat-label">所有虚拟机容量正常</span>
        </div>
      </div>
    </div>
    ...
    <div class="table-section">
      <div class="table-card glass-card">
        <div class="table-header">
          <h3 class="table-title">大容量虚拟机列表</h3>
        ...
        <div class="table-wrapper" v-if="data.data && data.data.length > 0">
          <table class="glass-table">
            ...
            <tbody>
              <tr v-for="(row, index) in data.data" :key="index" class="table-row large">
                <td class="vm-cell">
                  <svg viewBox="0 0 24 24" fill="none" class="vm-icon large">
                ...
                <td class="disk-cell">
                  <div class="disk-display">
                    <span class="disk-value">{{ row.disk_tb }}</span>
                    <span class="disk-unit">TB</span>
                    <div class="disk-bar">
                      <div class="disk-progress" :style="{ width: Math.min(row.disk_tb / 5 * 100, 100) + '%' }"></div>
                    </div>
                  </div>
                </td>
                ...
                <td class="owner-cell">
                  <span class="owner-badge">{{ row.owner }}</span>
                </td>
        ...
        <div class="empty-state" v-else>
          ...
          <h3 class="empty-title">所有虚拟机容量正常</h3>
          <p class="empty-desc">当前没有发现超过1TB容量阈值的虚拟机</p>
        </div>
      </div>
    </div>
  </div>
...
const loadData = async () => {
  try {
    const response = await axios.get('/api/inspection/periodic/large-vm')
    data.value = response.data
    animateValue(data.value.total, animatedTotal)
  } catch (error) {
    console.error('加载大容量虚拟机数据失败:', error)
  }
}
...
const exportData = () => {
  const csv = data.value.data.map(row =>
    [row.vm_name, row.ip, row.disk_tb, row.cpu, row.memory_gb, row.owner, row.system, row.function, row.action, formatTime(row.check_time)].join(',')
  )
  const blob = new Blob(['虚拟机名称,IP地址,磁盘容量TB,CPU,内存GB,负责人,系统,功能,处理建议,检查时间\n' + csv.join('\n')], { type: 'text/csv' })
  const url = URL.createObjectURL(blob)
  const a = document.createElement('a')
  a.href = url
  a.download = `large-vm-check-${new Date().toISOString().split('T')[0]}.csv`
  a.click()
}
...
.stat-card.large .stat-value { color: #fbbf24; }
.stat-card.threshold .stat-value { color: #60a5fa; font-size: 28px; }
